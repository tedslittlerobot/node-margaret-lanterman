...
test('events: opening', t => {
	const sections = new LantermanSectionRecorder();

	let called = 0;

	sections.events.on('section:opening', (instance: LantermanSectionRecorder) => {
		t.is(instance, sections);
		t.deepEqual(instance.current, {title: '', index: 0});
		called++;
	});
...
test('events: opened', t => {
	const sections = new LantermanSectionRecorder();

	let called = 0;

	sections.events.on('section:opened', (instance: LantermanSectionRecorder) => {
		t.is(instance, sections);
		t.deepEqual(instance.current, {title: 'foo', index: 0});
		called++;
	});
...
test('events: closing', t => {
	const sections = new LantermanSectionRecorder();

	let called = 0;

	sections.events.on('section:closing', (instance: LantermanSectionRecorder) => {
		t.is(instance, sections);
		t.deepEqual(instance.current, {title: 'foo', index: 0});
		called++;
	});
...
test('events: closed', t => {
	const sections = new LantermanSectionRecorder();

	let called = 0;

	sections.events.on('section:closed', (instance: LantermanSectionRecorder) => {
		t.is(instance, sections);
		t.deepEqual(instance.current, {title: '', index: 1});
		called++;
	});
